...
class FastDataset(Dataset):
    def __init__(self, folder_path, label_target, one_vs_rest):
        ...
        self.file_list = [
            ...
            if isfile(os.path.join(folder_path, f)) and (any(label in f for label in self.label_target))
        ]

        self.labels = []
        for f in self.file_list:
            file_name = os.path.basename(f)
            label = next((i for i, target in enumerate(self.label_target) if target in file_name))
